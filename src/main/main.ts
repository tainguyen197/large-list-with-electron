...
ipcMain.on(
  'get-content-with-path-enhance-with-resizing1',
    ...
    try {
      ...
      const sendPage = async () => {
        ...
        for (const fileName of pageData) {
          ...
          if (isImageType) {
            const { thumbSize, error } = await resizeService.resizeImage(
              filePath,
              workerIndex,
            );
            console.log('in loop: ', workerIndex);
            ...
            if (error) {
              console.error(
                `Error resizing image ${path.extname(fileName)}: ${error}`,
              );
              pageItems.push(initData);
            } else {
              pageItems.push({
        ...
        // const filteredPage = pageItems.filter((result) => result !== null);
        const endTime = now();
